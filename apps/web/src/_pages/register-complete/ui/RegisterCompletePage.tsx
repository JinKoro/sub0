...
import { SetPasswordForm } from '@/features/auth/set-password-form/ui/SetPasswordForm';
import { SidePanel } from '@/_pages/register/ui/SidePanel';

export function RegisterCompletePage({ token }: { token: string }) {
  const { t } = useLang();
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        >
          ...
          {!isMobile && (
            ...
            >
              {t('Уже подтвердили?', 'Already verified?')}
            </span>
          )}
          <Link
            href="/login"
