    ...
    <section style={{
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center', position: 'relative' }}>
        <div style={{
          ...
          fontSize: 12, fontFamily: mono, color: SUB0.blue,
          textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 28,
        }}>
          <span style={{ width: 6, height: 6, borderRadius: 999, background: SUB0.blue, display: 'inline-block' }} />
          {t('Начните за 2 минуты', 'Start in 2 minutes')}
        </div>

        <h2 style={{ fontSize: 72, fontWeight: 700, lineHeight: 1.03, letterSpacing: '-0.035em', margin: '0 0 20px' }}>
          {t('Начни контролировать подписки', 'Take control of your subscriptions')}<br />
          <span style={{ color: SUB0.blue }}>{t('уже сегодня.', 'today.')}</span>
        </h2>

        <p style={{ fontSize: 18, color: '#bfbfb8', maxWidth: 620, margin: '0 auto 40px', lineHeight: 1.5 }}>
          {t(
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <a href="#" className="s-btn" style={{
            ...
            padding: '18px 28px', background: SUB0.blue, color: '#fff',
            borderRadius: 10, fontWeight: 600, fontSize: 17, textDecoration: 'none',
          }}>
            {t('Начать бесплатно', 'Start free')} →
          </a>
          <a href="#" className="s-btn" style={{
            display: 'inline-flex', alignItems: 'center',
            padding: '18px 24px', background: 'transparent', color: SUB0.bg,
            border: '1px solid #2a2a2a', borderRadius: 10,
            fontWeight: 500, fontSize: 17, textDecoration: 'none',
          }}>
            {t('Связаться с командой', 'Book a call')}
          </a>
        </div>

        <div style={{ marginTop: 32, display: 'flex', gap: 28, justifyContent: 'center', flexWrap: 'wrap', fontFamily: mono, fontSize: 13, color: '#999' }}>
