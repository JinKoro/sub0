...
function MiniVisual({ kind, t }: { kind: VisualKind; t: (ru: string, en: string) => string }) {
  const box = {
    ...
    borderRadius: 8, height: 148, padding: 14,
      ...
      <div style={box}>
        {SUBS.slice(0, 4).map(r => (
          <div key={r.name} style={{
            ...
            padding: '6px 8px', background: SUB0.panel, borderRadius: 4,
          }}>
  ...
  if (kind === 'bell') {
    return (
      <div style={{ ...box, justifyContent: 'center', alignItems: 'center', gap: 14 }}>
        <div style={{
          width: 44, height: 44, borderRadius: 999, background: SUB0.blue, color: '#fff',
  ...
  return (
    <div style={{ ...box, gap: 6, justifyContent: 'center' }}>
      {projects.map(p => (
        <div key={p.ruName} style={{
          display: 'flex', alignItems: 'center', gap: 8, padding: '7px 9px',
          background: SUB0.panel, border: `1px solid ${p.active ? SUB0.ink : SUB0.line}`,
          borderRadius: 5, fontSize: 11,
        }}>
          <div style={{ width: 14, height: 11, borderRadius: 2, background: p.color, position: 'relative' }}>
            <div style={{ position: 'absolute', top: -3, left: 0, width: 6, height: 3, background: p.color, borderRadius: '2px 2px 0 0' }} />
          </div>
          <span style={{ fontWeight: 600, flex: 1 }}>{t(p.ruName, p.enName)}</span>
          <span style={{ fontFamily: mono, color: SUB0.muted }}>{p.count}</span>
        </div>
      ))}
      <div style={{ textAlign: 'center', fontSize: 10, color: SUB0.muted, fontFamily: mono, marginTop: 2, letterSpacing: '0.04em' }}>
...
const ITEMS: Array<{ num: string; ru: string; en: string; subRu: string; subEn: string; visual: VisualKind }> = [
  {
    num: '01', ru: 'Все подписки в одном месте', en: 'Every subscription in one place',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginTop: 48 }}>
        {ITEMS.map(it => (
          <div key={it.num} className="s-card" style={{
            ...
            padding: 0, overflow: 'hidden', background: SUB0.panel,
            display: 'flex', flexDirection: 'column',
          }}>
            <div style={{ padding: '22px 22px 0' }}>
              <div style={{ fontFamily: mono, fontSize: 11, color: SUB0.blue, letterSpacing: '0.1em', marginBottom: 14 }}>{it.num}</div>
              <div style={{ fontSize: 19, fontWeight: 700, lineHeight: 1.2, letterSpacing: '-0.02em', marginBottom: 10 }}>
                {t(it.ru, it.en)}
              </div>
