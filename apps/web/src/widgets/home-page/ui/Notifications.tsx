...
const CHANNELS = [
  ...
  {
    icon: '✈',
    ru: 'В Telegram и MAX', en: 'Telegram & MAX',
...
export function Notifications() {
  ...
  return (
    <Section bg={SUB0.bg} pad={isMobile ? '64px 20px' : '120px 48px'}>
      ...
      <p style={{ fontSize: 17, color: SUB0.muted, maxWidth: 520, lineHeight: 1.6, margin: '16px 0 56px' }}>
        {t('Уведомления помогут вовремя среагировать на любое списание.', 'Notifications help you react to any charge in time.')}
      </p>
      ...
      <div style={{
        ...
        {CHANNELS.map((c, i) => (
          <div key={i} style={{
            background: SUB0.panel, border: `1px solid ${SUB0.line}`, borderRadius: 14,
            padding: '32px 28px', display: 'flex', flexDirection: 'column', gap: 20,
          }}>
            <div style={{
              height: 48, width: 48, borderRadius: 12, background: ICON_BG[i],
            ...
            <div>
              ...
              <div style={{ fontSize: 15, lineHeight: 1.6, color: '#555' }}>{t(c.subRu, c.subEn)}</div>
            </div>
          </div>
        ))}
      </div>
